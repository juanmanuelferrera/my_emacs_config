...
;;   ("M-s f" . find-name-dired)
  ...
  (set-face-attribute 'default nil :family "iA Writer Duo S" :height 200 :weight 'regular)

  (when (eq system-type 'darwin)
    (setq insert-directory-program "gls")
    (setq mac-command-modifier 'meta)
    (set-face-attribute 'default nil :family "RobotoMono Nerd Font" :height 140))
...
(setq consult-denote-display-format "%t %k")

(defun ez/find-denote-file-by-short-name ()
  "Find and open a file in denote-directory by its short name.
Lists files in `denote-directory`, displaying only the part after '--'.
Filters the list by typing and opens the selected file upon Enter."
  (interactive)
  (let* ((dir denote-directory)
         (file-alist '()) ; To store pairs: (short-name . full-path)
         (prompt "Find denote file (short name): "))
    ;; --- Input Validation ---
    (unless (boundp 'denote-directory)
      (error "The variable 'denote-directory' is not defined."))
    (unless dir
      (setq dir default-directory)
      (message "Using default-directory: %s" dir)) ; This message is fine
    (unless (file-directory-p dir)
      (error "'%s' is not a valid directory." dir))

    ;; --- Build File List (wrapped) ---
    (with-temp-message "" ; Display nothing in echo area during this block
      (dolist (file (directory-files dir nil "^[^.]"))
        (let ((full-path (expand-file-name file dir)))
          (when (and (not (file-directory-p full-path))
                     (string-match "--" file))
            (let ((parts (split-string file "--")))
              (when (= (length parts) 2)
                (let ((short-name (nth 1 parts))
                      (actual-path full-path))
                  (push (cons short-name actual-path) file-alist))))))))

    ;; --- User Interaction ---
    (if (null file-alist)
        (message "No suitable denote files found in %s" dir)
      (let* ((selected-short-name (completing-read prompt file-alist nil t)))
        (when selected-short-name
          (let ((file-to-open (alist-get selected-short-name file-alist nil nil #'equal)))
            (when file-to-open
              (find-file file-to-open))))))))
;; denote
(bind-key* "M-\\" 'consult-denote-find)
(bind-key* "M-C-\\" 'denote-insert-link)
